...
const Analytics: React.FC = () => {
  const { data: properties, error: propertiesError, isLoading: isLoadingProperties } = useSWR<Property[]>('/api/properties');
  const { data: transactions, error: transactionsError, isLoading: isLoadingTransactions } = useSWR<Transaction[]>('/api/transactions');

  const isLoading = isLoadingProperties || isLoadingTransactions;
  const error = propertiesError || transactionsError;

  const [selectedPeriod, setSelectedPeriod] = React.useState('12months');
  const [selectedPropertyId, setSelectedPropertyId] = React.useState('all');
  const [propertyPage, setPropertyPage] = React.useState(0);
