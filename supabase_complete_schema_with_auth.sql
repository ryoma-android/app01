-- =====================================================
-- 不動産投資管理アプリ「次の一手」完全データベーススキーマ（認証含む）
...
-- 認証設定の確認（Supabase Dashboardで設定済みであることを前提）
-- 以下の設定は Supabase Dashboard > Authentication > Settings で確認・設定してください：

-- 1. Site URL: http://localhost:3000 (開発環境)
-- 2. Redirect URLs: 
--    - http://localhost:3000/auth/callback
--    - http://localhost:3000/auth/reset-password
-- 3. Email Templates: カスタマイズ可能
-- 4. SMTP Settings: 必要に応じて設定
...
CREATE TABLE IF NOT EXISTS public.auth_logs (
    ...
    event_type TEXT NOT NULL CHECK (event_type IN ('login', 'logout', 'signup', 'password_reset', 'failed_login')),
    ip_address INET,
    user_agent TEXT,
...
BEGIN
    INSERT INTO public.auth_logs (
        ...
        p_user_id, p_event_type, p_ip_address, p_user_agent, p_success, p_error_message
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- user_sessions テーブルのポリシー
CREATE POLICY "user_sessions_select_policy" ON public.user_sessions FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "user_sessions_insert_policy" ON public.user_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "user_sessions_update_policy" ON public.user_sessions FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "user_sessions_delete_policy" ON public.user_sessions FOR DELETE USING (auth.uid() = user_id);

-- auth_logs テーブルのポリシー（管理者のみアクセス可能）
CREATE POLICY "auth_logs_select_policy" ON public.auth_logs FOR SELECT USING (auth.uid() IN (
    SELECT id FROM auth.users WHERE email IN ('admin@example.com') -- 管理者メールアドレスを設定
));
CREATE POLICY "auth_logs_insert_policy" ON public.auth_logs FOR INSERT WITH CHECK (true); -- ログ記録は全ユーザー可能
...
-- 以下の設定を Supabase Dashboard > Authentication > Settings で確認・設定してください：
...
-- 3. Email Templates: カスタマイズ可能
-- 4. SMTP Settings: 必要に応じて設定
-- 5. Auth Providers: Email, Google, GitHub など
-- 6. Session Management: セッション時間の設定
-- 7. Password Policy: パスワード強度の設定
...
-- 4. Authentication設定を確認・調整
-- 5. アプリケーションでユーザー登録・ログインができるかテスト
-- 6. 各機能（物件管理、取引記録、AI提案など）が正常に動作するかテスト 